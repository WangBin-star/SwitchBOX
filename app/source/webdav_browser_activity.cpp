...
WebDavBrowserContentBuild build_webdav_browser_content_from_entries(
    const switchbox::core::WebDavSourceSettings& source,
    const std::string& relative_path,
    const std::vector<switchbox::core::WebDavBrowserEntry>& entries,
...
WebDavBrowserContentBuild build_webdav_browser_content_from_result(
    ...
    auto built = build_webdav_browser_content_from_entries(
        source,
        relative_path,
        rendered_entries,
...
WebDavBrowserContentBuild build_webdav_browser_content(
    ...
    return build_webdav_browser_content_from_result(
        source,
        relative_path,
        browse_result,
        preferred_focus_relative_path,
...
void WebDavBrowserActivity::request_focus_after_return(
    const switchbox::core::WebDavSourceSettings& source,
    std::string directory_relative_path,
    std::string focus_relative_path,
    std::string deleted_relative_path) {
    ...
    g_pending_delete_refresh.directory_relative_path =
        switchbox::core::webdav_join_relative_path({}, directory_relative_path);
    g_pending_delete_refresh.focus_relative_path =
        switchbox::core::webdav_join_relative_path({}, focus_relative_path);
    g_pending_delete_refresh.deleted_relative_path =
        switchbox::core::webdav_join_relative_path({}, deleted_relative_path);
}

void WebDavBrowserActivity::request_refresh_after_return(
    const switchbox::core::WebDavSourceSettings& source,
    std::string directory_relative_path,
    std::string focus_relative_path) {
    g_pending_delete_refresh.pending = true;
...
void WebDavBrowserActivity::request_delete_after_return(
    const switchbox::core::WebDavSourceSettings& source,
    std::string directory_relative_path,
    std::string focus_relative_path,
    std::string deleted_relative_path) {
...
brls::View* WebDavBrowserActivity::createContentView() {
    ...
    this->has_cached_entries = build.content_from_server;
    return build.view;
}
...
void WebDavBrowserActivity::willDisappear(bool resetState) {
    brls::Activity::willDisappear(resetState);
    if (!this->relative_path.empty()) {
        WebDavBrowserActivity::request_refresh_after_return(
            this->source,
            switchbox::core::webdav_parent_relative_path(this->relative_path),
            this->relative_path);
    }
}

void WebDavBrowserActivity::onResume() {
...
void WebDavBrowserActivity::update_focused_entry_state(const std::string& relative_path) {
    ...
    for (const auto& entry : this->cached_entries) {
        if (switchbox::core::webdav_join_relative_path({}, entry.relative_path) ==
...
void WebDavBrowserActivity::ensure_view_visible(brls::View* focus_target) {
...
bool WebDavBrowserActivity::restore_focus_now(const std::string& preferred_focus_relative_path) {
    const std::string normalized_focus =
        switchbox::core::webdav_join_relative_path({}, preferred_focus_relative_path);
...
void WebDavBrowserActivity::schedule_focus_restore(
    const std::string& preferred_focus_relative_path,
...
void WebDavBrowserActivity::apply_local_delete_result(
    ...
    this->cached_entries.erase(
        std::remove_if(
            ...
            [&normalized_deleted](const switchbox::core::WebDavBrowserEntry& entry) {
                return switchbox::core::webdav_join_relative_path({}, entry.relative_path) ==
                       normalized_deleted;
            }),
        this->cached_entries.end());
...
bool WebDavBrowserActivity::handle_back_action() {
    const std::string parent_relative_path =
        switchbox::core::webdav_parent_relative_path(this->relative_path);
    const std::string normalized_parent_relative_path =
        switchbox::core::webdav_join_relative_path({}, parent_relative_path);

    WebDavBrowserActivity* previous_browser = nullptr;
    const auto activities = brls::Application::getActivitiesStack();
    if (activities.size() >= 2) {
        previous_browser = dynamic_cast<WebDavBrowserActivity*>(activities[activities.size() - 2]);
    }

    const bool can_pop_to_previous_browser =
        previous_browser != nullptr &&
        same_webdav_source(previous_browser->source, this->source) &&
        switchbox::core::webdav_join_relative_path({}, previous_browser->relative_path) ==
...
std::string WebDavBrowserActivity::find_next_focus_after_delete(const std::string& deleted_relative_path) const {
    ...
    for (int index = 0; index < static_cast<int>(this->cached_entries.size()); ++index) {
        if (switchbox::core::webdav_join_relative_path(
                {},
...
void WebDavBrowserActivity::confirm_delete_focused_entry() {
    sync_focused_entry_from_ui();

    if (this->focused_entry_relative_path.empty()) {
        brls::Application::notify(tr("webdav_browser/empty"));
        return;
    }

    const std::string deleting_relative_path =
        switchbox::core::webdav_join_relative_path({}, this->focused_entry_relative_path);
    ...
    dialog->addButton(brls::getStr("hints/ok"), [this, deleting_relative_path]() {
        ...
        if (!switchbox::core::delete_webdav_file(this->source, deleting_relative_path, error_message)) {
            if (error_message.empty()) {
                error_message = "Failed to delete WebDAV file.";
            }
...
bool WebDavBrowserActivity::consume_pending_refresh_if_any() {
    if (!g_pending_delete_refresh.pending) {
        return false;
    }

    if (!same_webdav_source(g_pending_delete_refresh.source, this->source)) {
        return false;
    }
    ...
    const std::string normalized_directory =
        switchbox::core::webdav_join_relative_path({}, directory);
    const std::string normalized_current =
        switchbox::core::webdav_join_relative_path({}, this->relative_path);
...
bool WebDavBrowserActivity::apply_pending_return_from_player_if_any() {
    if (!g_pending_delete_refresh.pending) {
        return false;
    }
    ...
    if (kind == PendingRefreshKind::DeleteAfterPlayerExit) {
        std::string error_message;
        if (!switchbox::core::delete_webdav_file(this->source, hidden, error_message)) {
            if (error_message.empty()) {
                error_message = "Failed to delete WebDAV file.";
            }
            auto* failed = new brls::Dialog(error_message);
...
void WebDavBrowserActivity::refresh_after_player_delete(
    ...
    auto rebuilt = build_webdav_browser_content(
        this->source,
        this->relative_path,
        focus_relative_path,
