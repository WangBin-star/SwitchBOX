...
- 底部面板支持 8 个可聚焦控件：
  - 旋转、长退、短退、播放/暂停、短进、长进、音轨、字幕
- 在“旋转”上按 `A` 可让画面顺时针旋转 `90°`。
- 在“音轨/字幕”上按 `A` 可弹出选择列表并立即切换；
  - 若无可选项，会提示“无可选音轨 / 字幕”。

## 项目说明

截至 `2026-04-15`，项目已经从“仓库骨架 + 应用壳”阶段，推进到“设置系统、配置文件、多语言、SMB 浏览、SMB 播放与播放器交互落地”阶段。
...
- `Windows` 只是开发和调试目标
- 先把应用壳、设置、语言、来源配置打稳
- 再继续稳定真实播放壳与 SMB 流式链路，并推进 IPTV 播放流和播放历史

### 当前项目目标

当前开发按下面顺序推进：

1. 稳定 `Switch` 构建、打包与真机验证链路
2. 稳定 `SMB` 浏览与 `SMB` 流式播放链路
3. 推进 `IPTV` 实播与页面接入
4. 接入播放历史并继续打磨播放器细节
...
- 播放器底部进度面板已支持 8 个控件（含旋转、音轨、字幕选择）
- 播放器左侧文件浮窗、底部进度面板、音量浮窗均已接入
- `SMB` 排序方式已正式接入，并同时作用于浏览页和播放器左侧文件浮窗
- `show_hidden` 已从设置页与 `ini` 中移除，当前 `SMB` 浏览统一始终显示全部条目
...
当前程序已经不再只是“应用壳”，现阶段是“可启动、可切页、可改语言、可浏览 SMB、可播放 SMB 文件”的版本：
...
- 当前整体处于“设置、浏览、SMB 播放与播放器主要交互已接入，后续继续稳定流式播放并推进 IPTV / 播放历史”的阶段
...
- 普通设置保存后会刷新首页
- `排序方式` 已真实接入，会同时作用于 `SMB` 浏览页和播放器左侧文件浮窗
- `显示隐藏文件` 已移除，当前 `SMB` 浏览统一始终显示全部条目
- 在 `IPTV` / `SMB` 列表中：
  - 条目前的 `● / ○` 表示启用状态
  - `Y` 直接切换启用 / 停用
  - `X` 删除条目，并弹确认框
...
- `[general]` 当前不再保留 `show_hidden`
- `SMB` 浏览排序由 `[general].sort_order` 控制
- `title` 允许作为用户可见名称
- 实际分组名仍使用稳定的内部 `key`

### 当前已知问题

- `Switch` 端 `SMB` 流式播放链路仍需继续做兼容性、错误处理和性能稳定化
- 桌面端仍主要用于 UI / 流程调试，不作为真实播放性能判断依据
...
1. 继续稳定 `Switch` 端 `SMB` 流式播放链路
2. 继续打磨播放器细节交互与错误处理
3. 完成 `IPTV` 页面接入与实播链路落地
4. 接入播放历史，统一承接 `IPTV / SMB` 最近播放记录
